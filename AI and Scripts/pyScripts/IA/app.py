...
def predict():
    ...
    with torch.no_grad():
        ...
        genetalPrediction = getGeneralProbabilities(predicted_index)

        match(genetalPrediction):
            case "Corn":
                test_image = Image.open(file.stream).convert('RGB')
    ...
    return jsonify({'plant': genetalPrediction.lower(), 
                    'plant_raw_prediction': generalProbabilities.tolist(),
                    'prediction': predicted_class.lower(), 
                    'raw_prediction': probabilities.tolist()}), 200
